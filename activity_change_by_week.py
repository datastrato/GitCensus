#
# Work out all users activity changes week by week
...
for no in range(52):
    for user in users:
        if no > 0:
            this_week = 0
            if user in users:
                this_week = users[user][no]
            last_week = 0
            if user in users:
                last_week = users[user][last]
            if this_week > 0 and last_week > 0:
                if abs(this_week/last_week-1) > 0.5:
                    print(no,user,this_week/last_week)
